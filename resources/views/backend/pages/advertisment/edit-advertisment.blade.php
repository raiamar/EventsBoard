    ...
    <main class="h-full overflow-y-auto">
        <div class="container px-6 mx-auto grid">
            ...
            <div class="w-full overflow-hidden rounded-lg shadow-xs">
                ...
                <form class="w-full p-10 mx-auto" action="{{ route('adds.update', $data->id) }}" method="POST"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <x-inline-form-input label="Redirect Link" required="*" value="{{$data->redirect_link}}"
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <x-form-select name="type" label="Advertisment Type">
                            @foreach (AddType::getAllAddType() as $key => $value)
                                <option value="{{ $value }}" @if($value==$data->type) selected @endif>{{ $key }}</option>
                            @endforeach
                        </x-form-select>
                        ...
                        <x-form-select name="page" label="Advertisment Page">
                            @foreach (AddPageType::getAllAddPageType() as $key => $value)
                                <option value="{{ $value }}" @selected($value == $data->page)>{{ $key }}</option>
                            @endforeach
                        </x-form-select>
                    </div>

                    @if($data->page==$value) selected @endif
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <x-form-input label="Order" required="*" value="{{$data->order}}" type="number"
                            name="order" />
                    </div>
